...
export default function AppDetailPage() {
  ...
  useEffect(() => {
    const load = async () => {
      if (id.startsWith('fb_')) {
        const realId = id.replace('fb_', '');
        try {
          const snap = await getDoc(doc(db, 'apps', realId));
          if (snap.exists()) {
            setApp({
              id,
              ...snap.data(),
              color: snap.data().color || '#C62828',
              banner: snap.data().banner || 'linear-gradient(135deg, #7F0000, #C62828)',
              rating: 0, reviews: 0, downloads: '0',
              screenshots: snap.data().screenshots || [],
              oldVersions: [],
            });
          }
        } catch(e) { console.error(e); }
      } else {
        setApp(getAppById(id));
      }
      setLoading(false);
    };
    load();
  ...
  const media = getMedia(app.version);
        ...
        <div className="screenshots-scroll">
          {media?.screenshots
            ? media.screenshots.map((url, i) => (
                <div key={i} className="screenshot screenshot-img">
                  <img
                    src={url}
                    alt={`Pantalla ${i+1}`}
                    style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
