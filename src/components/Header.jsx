...
import { Search, X, Moon, Sun, ChevronLeft, ShoppingBag, Palette } from 'lucide-react';
import { useTheme } from '../context/ThemeContext';
import { searchApps } from '../data/apps';
import './Header.css';

export default function Header() {
  const { theme, toggle, accentId, setAccent, accents } = useTheme();
          ...
          <>
            <div className="header-left">
              ...
              <Link to="/" className="header-logo">
                ...
                <span className="logo-text">Angrybirds Store</span>
              </Link>
            </div>
            <div className="header-right">
              ...
              <div className="color-picker-wrap">
                <button className="header-icon-btn" onClick={() => setColorOpen(o => !o)} aria-label="Color">
                  <Palette size={22} />
                </button>
                {colorOpen && (
                  <div className="color-dropdown">
                    {accents.map(a => (
                      <button
                        key={a.id}
                        className={`color-dot ${accentId === a.id ? 'selected' : ''}`}
                        style={{ background: a.header }}
                        title={a.label}
                        onClick={() => { setAccent(a.id); setColorOpen(false); }}
                      />
                    ))}
                  </div>
                )}
